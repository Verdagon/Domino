...
public class Root : MonoBehaviour {
    ...
    (GameObject, GameObject) MakeTile(Pattern pattern, int patternTileIndex, Vector3 position) {
        var shapeIndex = pattern.patternTiles[patternTileIndex].shapeIndex;
        
        var (tileMesh, outlinesMesh) = tileShapeMeshCache.Get(shapeIndex, .3f, .025f);
        
        var facesObject = Instantiate(Resources.Load("Quad")) as GameObject;
        facesObject.GetComponent<MeshRenderer>().sharedMaterial = white;
        facesObject.GetComponent<MeshFilter>().sharedMesh = tileMesh;

        var outlinesObject = Instantiate(Resources.Load("Quad")) as GameObject;
        outlinesObject.GetComponent<MeshRenderer>().sharedMaterial = black;
        ...
        facesObject.transform.rotation = rotation;
        facesObject.transform.position = position;
        // facesObject.isStatic = true;
        outlinesObject.transform.rotation = rotation;
        outlinesObject.transform.position = position;
        // outlinesObject.isStatic = true;
        // var tileObject = Instantiate(Resources.Load("EmptyGameObject"), position, rotation) as GameObject;
        // facesObject.transform.SetParent(tileObject.transform, false);
        // outlinesObject.transform.SetParent(tileObject.transform, false);
        // return tileObject;
    ...
    void Start() {
        ...
        black = Instantiate(Resources.Load("BlackWithInstancing")) as Material;
        white = Instantiate(Resources.Load("WhiteWithInstancing")) as Material;
        ...
        tileShapeMeshCache = new TileShapeMeshCache(pattern);
        for (int groupX = 0; groupX < 5; groupX++) {
            for (int groupY = 0; groupY < 5; groupY++) {
                for (int tileIndex = 0; tileIndex < pattern.patternTiles.Count; tileIndex++) {
                    for (int elevation = 0; elevation < pattern.patternTiles.Count; elevation++) {
                        var translateVec2 = pattern.GetTileCenter(new Location(groupX, groupY, tileIndex));
                        var translateUPos = translateVec2.ToVec3().ToUnity();
                        translateUPos.y += elevation * .3f;
                        var (tileObject, outlineObject) = MakeTile(pattern, tileIndex, translateUPos);
